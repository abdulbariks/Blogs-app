...
const Profile = ({ setLoggedIn }) => {
  ...
  const fields =
    password === ""
      ? { name, email, profileDP }
      : { name, email, password, profileDP };
  ...
  const imageUpload = async (event) => {
    ...
    if (file && file.size <= MAX_FILE_SIZE && file.type.startsWith("image/")) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        setMessage(
          "The selected file is too large. Please select a file that is smaller than 2MB."
        );
        setShowPopup(true);
    ...
    <>
      <div className="profile">
        <div className="profileWrapper">
          <i
            className="fas fa-times-circle profileCloseIcon"
            onClick={() => {
              navigate(-1);
            }}
          ></i>
          ...
          <form className="profileForm" onSubmit={submitHandler}>
            ...
            <div className="profilePP">
              ...
              <label htmlFor="removeProfileDP">
                <i
                  className="profilePPIcon fas fa-trash-alt"
                  style={{ color: "red" }}
                ></i>
              </label>
              <input
              ...
              {showConfirmPopup && (
                <div className="popups">
                  <div className="popups-content">
                    <h2>
                      Are you sure ? All of your posts will also be deleted
                    </h2>
                    <div className="popup-buttons">
                      <button
                        className="popup-ok-button popupButton"
                        onClick={deleteHandler}
                      >
                        Delete
                      </button>
                      <button
                        className="popup-cancel-button popupButton"
